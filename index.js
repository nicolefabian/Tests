...
const exp = require('constants');
const {Builder, Browser, By} = require('selenium-webdriver'); //node .\index.js to run the script
var {login, sanityCheckPageLoad,sanityCheckInvalidLogin, readData} = require('./function'); //importing the login function from function.js

var tc001 = async function() {
    //initiate the browser
    let driver = await new Builder().forBrowser(Browser.FIREFOX).build();
    tcData = await readData("tc001"); //returns the data for tc001 
    try {
        //login function -> pass driver
        console.log("TC001:Login started");
...
var tc002 = async function() {
    //initiate the browser
    let driver = await new Builder().forBrowser(Browser.FIREFOX).build();
    tcData = await readData("tc002"); //returns the data for tc001 
    try {
        //login function -> pass driver
        console.log("TC002:Login started");
        await login(driver,tcData.data.username,tcData.data.password);
